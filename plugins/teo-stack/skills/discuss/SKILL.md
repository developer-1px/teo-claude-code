...
6. 사용자가 묶음을 통과시키면 다음 턴부터 해당 요소는 별도 라벨 없이 일반 `🟢`로 둔다.
...
이번 턴에 새로 80% 이상이 된 요소가 있으면 이해도 뒤에 `신규` 또는 `신규-대기`만 표시한다. 이미 통과된 요소에는 별도 상태 라벨을 붙이지 않는다. `신규`가 하나라도 있으면 표 다음의 첫 섹션은 반드시 `이번 턴 통과 후보 근거`다.
