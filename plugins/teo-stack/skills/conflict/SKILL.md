---
name: conflict
description: 두 방향이 양립 불가능해 보일 때 대립 해소 다이어그램(Evaporating Cloud)으로 숨겨진 전제를 찾아 돌파구(Injection)를 도출한다. "A vs B", "이러지도 저러지도", "트레이드오프", "딜레마", "양립 불가", "/conflict" 등을 말할 때 사용. 옹호자 A/B 서브에이전트로 양쪽 전제를 각각 최대한 강하게 세운 뒤, 중재자가 깨지는 지점을 찾는다.
---
...
- **단독 호출** — `/conflict` 또는 "A vs B", "딜레마", "양립 불가" 같은 신호로 진입
- **`/discuss` 드릴다운** — 상위 논의에서 제약·전제가 양립 불가로 교착됐을 때 이 스킬로 드릴다운, 결과(깨진 전제 + Injection)를 상위 논의에 반영하여 복귀
...
결과를 대화에서 직접 제시한다. `/discuss` 드릴다운으로 진입한 경우에는 깨진 전제를 상위 논의의 "전제" 요소에, Injection을 "목표" 또는 "해결" 요소에 반영하여 복귀한다.
