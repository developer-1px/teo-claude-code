...
1. **기존 관행 감지** — repo 루트에서 `docs/`, `documentation/`, `doc/`, `wiki/` 등 문서 폴더가 있는지 확인한다. 있다면 그 하위의 파일 네이밍/폴더 규칙(평탄 구조인지, 날짜 폴더인지, 카테고리별인지)을 1~2개 샘플로 파악해 따라간다.
2. **관행이 없거나 모호하면 제안 후 확인** — 디폴트 제안: `docs/explain/YYYY-MM-DD-{slug}.md`. 사용자에게 경로와 파일명을 한 번 확인받은 뒤 저장한다.
3. **문서 폴더를 만들고 싶지 않다는 신호가 있으면** — 대화 내 직접 출력으로 대체. 파일은 생성하지 않는다.
