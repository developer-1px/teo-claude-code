---
name: srp
description: 파일 단위 단일 책임 원칙(SRP)을 점검하고 리팩토링한다. "단일 책임", "책임 분리", "파일이 너무 크다", "이 파일 쪼개자", "/srp", "/srp 300", "/goal srp 300" 등에 사용한다. 숫자는 import/export-from을 제외한 후보 발견 LOC 하한일 뿐 목표 크기가 아니다. 독립 변경 이유가 둘 이상이고 분리가 locality를 해치지 않을 때만 분리한다.
---
...
- LOC는 후보 발견에만 사용하고, 최종 판단은 독립 변경 이유, 파일명-구현 일치, 분리 후 locality로 한다.
- 한 파일이 300줄을 넘어도 실제 책임이 1개면 유지한다.
- 분리 후 새 파일이 300줄을 넘더라도 책임이 1개면 허용한다.

정식 판정식:

| 실제 책임 | 독립 이동 가능성 | 분리 후 locality | 판정 |
|---|---|---|---|
| 1개 | 해당 없음 | 유지 | 유지 |
| 2개 이상 | 높음 | 유지 또는 개선 | 분리 |
| 2개 이상 | 낮음 | 악화 | 한 deep module로 유지하거나 내부 구조만 정리 |
| 판단 불가 | 불명확 | 불명확 | 보류 |

책임 개수만으로 파일 수를 늘리지 않는다. 분리된 파일이 서로의 내부를 계속 알아야 하거나 함께 열어야 이해된다면 분리는 SRP 개선이 아니다.
...
6. 책임이 독립적으로 이동 가능한지, 분리 후 import와 caller knowledge가 줄어드는지 확인한다.
7. 실제 책임 수가 2개 이상이고 locality가 유지되거나 개선될 때만 분리 계획 표를 작성한다.
8. 분리가 shallow module이나 왕복 import를 만들면 유지 또는 내부 정리로 판정한다.
9. 애매하면 보류로 분류하고 판단 조건을 남긴다.
