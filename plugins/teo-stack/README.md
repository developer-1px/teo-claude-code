...
interactive-os 프로젝트를 위한 파이프라인 스킬·에이전트 모음. 프로젝트 고유의 구조(`os 부품`, `ax()` 12축, `FlatLayout`, PARA 문서 규약, `L1/L2/L3` 이해도 레벨 등)를 전제합니다.

> 이 플러그인은 interactive-os 컨벤션에 맞춰 작성됐습니다. 다른 프로젝트에서는 그대로 쓰기 어렵습니다. **프로젝트 중립 스킬이 필요하면 [`teo-universal`](../universal/) 사용.**
...
|-------|------|------|------------|
| `/discuss` | Structured work discussion | 업무 논의 구조화 | 요청이 모호할 때. TOC 13요소로 정리하여 방향 맞춤 |
| `/story` | User story map builder | 유저스토리 맵 인터뷰 | discuss 이후, PRD 이전. 구현 단위를 사용자 체감 언어로 도출 |
