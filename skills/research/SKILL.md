...
- **Agent A — MEMORY/CLAUDE.md**: 관련 규약·feedback·이미 결정된 방향 추출
- **Agent B — 코드베이스**: `src/interactive-os/CATALOG.md` 우선, 관련 부품·패턴·훅 스캔
- **Agent C — docs**: 이전 조사·PRD·아카이브(`docs/**/*.md`)에서 중복 여부 확인
...
**규약 충돌 사전 경고 필수**: 의뢰가 MEMORY의 기존 결정과 어긋나면 여기서 플래그. 예: "사용자가 shadcn 기반 BP를 물었으나 프로젝트는 ax() 24축 시스템으로 shadcn 원리만 흡수(`project_ax_shadcn_insight`). 외부 조사는 원리 비교 차원에서만 수행."
...
## Insight
AI의 한 줄 결론 + **프로젝트 규약과의 정합성 판정** (일치/부분 충돌/완전 충돌).
...
경로: `docs/YYYY/YYYY-MM/YYYY-MM-DD/{slug}.md` (프로젝트 md 규약)
...
- **규약 충돌 시 프로젝트 규약 우선** (`feedback_judgment_priority`). 외부 BP가 프로젝트 결정과 충돌하면 Insight에 명시하되 "표준이니까 바꾸자"고 주장하지 않는다.
