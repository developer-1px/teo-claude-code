...
각 레이어의 Area MDX는 mddb 쿼리 `type=area, layer=<layer>` (single, 있으면 갱신/없으면 생성). 생성 경로는 오늘 날짜 폴더 아래 `{layer}Area.mdx`.

| 경로 패턴 | 레이어 (frontmatter `layer`) |
|-----------|-----------------------------|
| `interactive-os/core/` | core |
| `interactive-os/axes/` | axes |
| `interactive-os/behaviors/` | patterns |
| `interactive-os/plugins/` | plugins |
| `interactive-os/hooks/` | hooks |
| `interactive-os/ui/` | ui |
| `pages/` | pages |
...
mddb 쿼리 `type=area, layer=axes` 결과를 참고하여 해당 레이어의 L2 Area MDX를 새로 생성한다 (`docs/YYYY/YYYY-MM/YYYY-MM-DD/{layer}Area.mdx` + frontmatter `type: area, layer: <layer>`). 구조:
...
발견된 빈칸은:
1. L2 주기율표에 ⬜ 행으로 추가
2. L3 최소 MDX 생성 (개념 + TODO) — `docs/{오늘 날짜 폴더}/{name}Area.mdx` + frontmatter `type: area, layer: <layer>`
3. 새 backlog 파일 생성 — `docs/{오늘 날짜 폴더}/{slug}Backlog.md` + frontmatter `type: backlog`
...
### 변경된 Area
- `axesArea.mdx` (type=area, layer=axes): value ⬜→🟢
- `valueArea.mdx` (type=area, layer=axes): L3 생성

### 새로 발견된 빈칸
- `axesArea.mdx`: trigger↔popup ⬜ (신규)
