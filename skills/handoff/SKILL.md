---
name: handoff
description: 작업을 깔끔하게 마무리하거나 다음 세션으로 이어 넘기는 단일 오케스트레이터. **retrospect+역PRD 감사(A0)** → verify(typecheck/lint/test/simplify/naming-audit) → commit → push → 남은 것 backlog화 → handoff 문서 생성을 한 번에 처리한다. Blueprint PRD가 있으면 역PRD 체크리스트를 먼저 채우고 Blueprint ⊃ Implementation 검증을 통과해야 나머지 단계가 진행된다. 세션 시작 시 `/handoff`를 치면 가장 최근 미소비 handoff를 이어받는다. "/handoff", "마무리", "닫자", "정리하자", "여기까지", "다음에 이어서", "중단" 등 명시적 의도 표현 시 사용. **사용자가 명시적으로 호출할 때만 실행한다. AI가 자율 판단으로 발동하지 않는다.**
---
...
### A0. Retrospect + 역PRD 감사 (**필수 선행 게이트**)

verify·commit·push 이전에 **이 세션의 작업이 Blueprint PRD와 정합하는지** 먼저 회고한다. 역PRD 체크리스트가 비어 있거나 위반이 있으면 handoff는 여기서 멈춘다. Blueprint ⊃ Implementation 불변을 지키는 것이 handoff의 1차 책임이다.

#### A0-1. 활성 Blueprint 탐지

세션 중 참조되거나 수정된 `docs/1-projects/**/prds/*-prd.md` 또는 `docs/2-areas/**/prds/*-prd.md` 파일을 나열한다. 없으면 A0 전체를 건너뛰고 A1로 진행.

#### A0-2. 역PRD 체크리스트 채우기 (§7)

각 Blueprint의 §7 역PRD 체크리스트를 실제 구현 상태로 채운다:

| 대상 섹션 | 확인 내용 | 채울 필드 |
|----------|----------|----------|
| 데이터 (§1) | 타입이 실제 존재 | `file::TypeName` |
| 파일 (§2) | 파일 존재 + LOC | 실제 경로 + 라인 수 |
| Export (§3) | export 존재 + 시그니처 일치 | `file::exportName` + 일치 여부 |
| 흐름 (§4) | 구현과 pseudo-code 차이 | diff 요약 (없으면 "그대로") |
| 경계 (§5) | 각 경계 처리 여부 | ✅ / ❌ + 비고 |
...
- **§2 반증**: 파일 맵에 없는 경로에 구현이 나타났는가? (grep으로 신규 파일 감지)
- **§3 반증**: §3에 없는 export가 구현에 등장했는가? (grep export)
- **§4 반증**: 흐름도에 없는 경로가 있는가?
- **§5 반증**: 모든 §5 경계가 §6 시나리오로 매핑되는가?
...
| 판정 | 조건 | 다음 |
|------|------|------|
| 🟢 통과 | 체크리스트 모두 채워지고 반증 조건 위반 0 | A1로 진행 |
| 🟡 경미 편차 | 일부 `(?)` 또는 Phase 1 skip 항목 존재 | 사용자에게 보고 후 승인 받아 A1로 |
| 🔴 위반 | 반증 조건 발동 (예: §3에 없는 export 발생) | **handoff 중단**, 수정 또는 Blueprint 갱신 후 재시도 |

#### A0-5. 보고 형식

```markdown
## A0 Retrospect + 역PRD 감사

- Blueprint: `docs/.../xxx-prd.md`
- §1 데이터: N/N 🟢
- §2 파일: N/N 🟢  (위반: 없음 / [파일맵 밖 파일 감지])
- §3 Export: N/N 🟢
- §4 흐름: diff 요약
- §5 경계: N/N (Phase 1 out: K)
- §6 검증: N/N 🟢

종합: 🟢 / 🟡 (사유) / 🔴 (사유 + 수정 제안)
```

🔴이면 여기서 멈춘다. 🟢/🟡 승인 시에만 A1 진입.
...
## 이어받는 법 (다음 세션 AI가 읽을 지시문 — 현재 사용자에게 말하지 말 것)
...
**현재 세션은 계속 이어갈 수 있습니다.** handoff 문서는 세션이 교체될 때만 의미가 있습니다 — 다음 세션 시작 시 `/handoff`를 치면 Step B가 이 파일을 자동으로 집어갑니다. 지금 중단하지 않으셔도 됩니다.
```
