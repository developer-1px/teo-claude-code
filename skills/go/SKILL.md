---
name: go
description: 자율 실행 오케스트레이터. /team 편성표를 소비하여 TaskCreate + Agent 디스패치로 작업을 자율 완주한다. "/go", "실행해", "만들어줘" 등 구현 요청 시 사용.
---

## 역할

대화 컨텍스트(discussion, PRD, /team 편성표)를 바탕으로 **task를 만들고 에이전트를 디스패치**하여 자율 완주한다. 메인은 직접 코드를 쓰지 않고 오케스트레이션한다.
...
1. **활성 PRD** — mddb 쿼리 `type=prd` 중 역PRD 열이 비어있는 PRD (project 또는 layer 필드로 범위 지정)
   - 1개 → 해당 PRD 기반으로 자동 진행
   - 여러 개 → `created` 날짜 순서대로 처리
2. **미소비 handoff** — mddb 쿼리 `type=handoff, status!=consumed` (가장 최근 1개 + 내부 "이후" 섹션의 backlog 링크)
...
`/team` 스킬로 팀 편성표를 산출한다. /team이 산출하는 것:
1. **필수 3역할** — 계획/실행/평가 에이전트 (과제 규모 무관 필수)
2. **특화 페르소나** — 과제 도메인 전문가 + 긴장 축 페르소나
3. **병렬성 분석** — 배치 분할 (독립 작업 식별)
4. **실행 모드** — 메인 단독 / 병렬 팀 / 단일 팀 / 대화 루프
...
/team 편성표의 배치를 TaskCreate로 변환한다:
...
- **/team 편성표의 평가 페르소나를 프롬프트에 반영**한다
- 합격 시 → Phase: Verify로 진행
- 5회 불합격 시 → 불합격 사유를 사용자에게 보고하고 판단을 구한다

---

PRD 없는 작업은 Execute 시작 전에 `docs/YYYY/YYYY-MM/YYYY-MM-DD/{name}Task.md` (frontmatter: `type: plan, project 또는 layer`)를 작성한다.
...
### Step 2: /simplify (에이전트)
- Agent로 simplify 실행 — git diff를 넘기고 코드 품질/재사용/효율 리뷰+수정
- 수정 발생 → Step 1 재검증

### Step 3: 커밋

### Step 4: /handoff 자동 진행
- Verify 완료 후 사용자에게 묻지 않고 바로 `/handoff`(나가는 길)로 진행한다
- `/handoff`가 commit, push, 남은 것 backlog화, handoff 문서 생성을 한 번에 처리한다. retro 산출물이 있으면 PROGRESS.md/ARCHITECTURE.md 갱신도 포함
